...
public class HotKeyEntryByCode : HotKeyEntry
{
    ...
    public HotKeyEntryByCode(ModCode modCodes, Code code, string excludeSelector, string? description, Func<HotKeyEntryByCode, ValueTask> action)
        : base(null, HotKeyMode.ByCode, typeof(ModCode), (int)modCodes, code.ToString(), excludeSelector, description, action.Target as IHandleEvent)
    {
    ...
    internal HotKeyEntryByCode(ILogger logger, ModCode modCodes, Code code, string excludeSelector, string? description, Func<HotKeyEntryByCode, ValueTask> action, IHandleEvent? ownerOfAction)
        : base(logger, HotKeyMode.ByCode, typeof(ModCode), (int)modCodes, code.ToString(), excludeSelector, description, ownerOfAction)
    {
        this.Modifiers = modCodes;
        this.Code = code;
        this._Action = action;
